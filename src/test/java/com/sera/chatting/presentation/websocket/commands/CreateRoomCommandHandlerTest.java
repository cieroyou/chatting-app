...
class CreateRoomCommandHandlerTest {
	...
	void supports_ShouldReturnTrue_ForCorrectCommand() {
		assertTrue(createRoomCommandHandler.supports(CREATE_COMMAND), "supports should return true for 'create_room' command");
	}

	@DisplayName("supports 메소드는 잘못된 커맨드에 대해 false를 반환한다")
	@Test
	void supports_ShouldReturnFalse_ForIncorrectCommand() {
		assertFalse(createRoomCommandHandler.supports(WRONG_COMMAND), "supports should return false for commands other than 'create_room'");
	}
	...
	void handleCommand_ShouldReturnSuccessAckBody_WhenCalledWithValidRequest() {
		...
		// Mock behavior
		when(chattingRoomFacade.createRoom(anyString())).thenReturn(expectedResponse);
		...
		verify(chattingRoomFacade).createRoom(expectedRoomName);
	}
}